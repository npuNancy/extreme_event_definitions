...
>
> 场站级生产 pipeline 当前使用 Regional BCSD 的 `wind_ms`/`rsds` 作为低资源输入，
> 2015-2024 为基线；与普通事件在统一 step2 中计算。本文通用算法说明不代表 CF 输入。
...
lr = low_resource_signal(resource, time, pct=5.0,
                         base_mask=base_2015_2024,   # 场站级 BCSD pipeline 使用固定基线
                         night=night_mask_or_None)
```

## 四、注意
- **低资源**最特殊：需 24h 居中滚动 + 月×时气候态(clim288)距平 + 每站 P5。场站级 BCSD
  pipeline 固定使用 2015-2024 基线；3 小时数据使用 8 个时间步；光伏务必传 `night`。
