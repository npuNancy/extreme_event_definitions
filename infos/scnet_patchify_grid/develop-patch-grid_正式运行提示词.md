...
先执行运行前准备的输入、账号、目录、配置与作业包步骤；本轮跳过goal及运行前准备中的pilot，完成准备即全量提交三模式ready任务。18worker在各自home更新develop-patch-grid：保留脏目录改动，本地测试、commit/push后经HTTPS clone或fast-forward更新，固定同一完整SHA与干净checkout。缺HTTPS认证就报告。运行期不改远程checkout；科学Python仅在计算节点运行，登录节点只做轻量操作。
...
中心runtime保存不可变release、模型选择、台账和提交锁。验证共同挂载、跨账号原子rename及flock互斥；输入只读，输出不共写。检查share容量、inode和实际quota，不能假定share无限，也不能只按最终NC估算；parts、merge临时文件及失败副本都计入空间。容量预测按已有实测及本轮产物更新，区分陆地点与矩形网格。
...
直接按作业分工全量运行三模式，不单独选代表patch，不等待基线或五年信号段的试运行。baseline/signals用8CPU/4worker、tile32×32、time_chunk240；audit用2CPU。运行中记录elapsed、内存、最终大小、parts与merge时间，至少留20%内存余量。资源不足时调整profile，在18worker新目录生成同一新pack，不覆盖已提交脚本；已有成功unit不重复跑。

固定每15分钟监控一次，Asia/Shanghai按整点、15分、30分、45分对齐；覆盖goal中按耗时调整间隔的规则。每轮记录检查时间和next_check_at。
